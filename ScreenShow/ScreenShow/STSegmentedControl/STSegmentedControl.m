...
@synthesize normalTextColor = _normalTextColor, selectedTextColor = _selectedTextColor, 
            leftCapWidth = _leftCapWidth, normalTextShadowColor = _normalTextShadowColor,
            selectedTextShadowColor = _selectedTextShadowColor,rightCapWidth = _rightCapWidth,
            middleCapWidth = _middleCapWidth;
...
- (id)initWithCoder:(NSCoder *)decoder {
    ...
    if(self) {
		self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, self.frame.size.width, STSegmentedControlHeight - 5);
...
-(void)initial{
    ...
    _leftCapWidth = 8;
    _rightCapWidth = _middleCapWidth = 2;
    _normalTextColor = [UIColor darkGrayColor];
    _selectedTextColor = [UIColor darkGrayColor];
    _normalTextShadowColor = [[UIColor alloc] initWithWhite:1 alpha:1];
    _selectedTextShadowColor = [[UIColor alloc] initWithWhite:1 alpha:1];
}
		...
		for(NSObject *object in segments)
		{
			...
			if(indexOfObject == 0)
			{
				if(selectedSegmentIndex == indexOfObject)
					[button setBackgroundImage:[selectedImageLeft stretchableImageWithLeftCapWidth:_leftCapWidth topCapHeight:0] forState:UIControlStateNormal];
				else
					[button setBackgroundImage:[normalImageLeft stretchableImageWithLeftCapWidth:_leftCapWidth topCapHeight:0] forState:UIControlStateNormal];
			}
			else if(indexOfObject == numberOfSegments - 1)
			{
				if(selectedSegmentIndex == indexOfObject)
					[button setBackgroundImage:[selectedImageRight stretchableImageWithLeftCapWidth:_rightCapWidth topCapHeight:0] forState:UIControlStateNormal];
				else
					[button setBackgroundImage:[normalImageRight stretchableImageWithLeftCapWidth:_rightCapWidth topCapHeight:0] forState:UIControlStateNormal];
			}
			else
			{
				if(selectedSegmentIndex == indexOfObject)
					[button setBackgroundImage:[selectedImageMiddle stretchableImageWithLeftCapWidth:_middleCapWidth topCapHeight:0] forState:UIControlStateNormal];
				else
					[button setBackgroundImage:[normalImageMiddle stretchableImageWithLeftCapWidth:_middleCapWidth topCapHeight:0] forState:UIControlStateNormal];
			}
            ...
            [button setTitleColor:_selectedTextColor forState:UIControlStateHighlighted];
			button.titleLabel.font = [UIFont boldSystemFontOfSize:14];
			button.titleLabel.shadowOffset = CGSizeMake(0, 1);
            [button setTitleShadowColor:_normalTextShadowColor forState:UIControlStateNormal];
            [button setTitleShadowColor:_selectedTextShadowColor forState:UIControlStateSelected];
            [button setTitleShadowColor:_selectedTextShadowColor forState:UIControlStateHighlighted];
...
- (void)segmentTapped:(UIButton *)button {
    if (selectedSegmentIndex == button.tag - 1 && button.selected) {
        return;
    }
...
- (void)insertSegmentWithObject:(NSObject *)object atIndex:(NSInteger)index {
	/*
...
- (void)setObject:(NSObject *)object forSegmentAtIndex:(NSInteger)index {
	/*
	 Making sure we don't call out of bounds
	 */
	if(index < numberOfSegments && index >= 0)
...
- (void)dealloc {
    ...
    [_selectedTextColor release];
    [_normalTextShadowColor release];
    [_selectedTextShadowColor release];
